...
//显示设备信息
    ...
    ioctl(mV4l2_fd,VIDIOC_QUERYCAP,&cap);
    qDebug("DriverName:%s/nCard Name:%s/nBus info:%s/nDriverVersion:%u.%u.%u/n",cap.driver,cap.card,cap.bus_info,(cap.version>>16)&0XFF,(cap.version>>8)&0XFF,cap.version&0XFF);
//显示所有支持的帧格式

    struct v4l2_fmtdesc fmtdesc;
    fmtdesc.index=0;
    fmtdesc.type=V4L2_BUF_TYPE_VIDEO_CAPTURE;
    while(ioctl(mV4l2_fd,VIDIOC_ENUM_FMT,&fmtdesc)!=-1)
    {
    qDebug("/t%d.%s/n",fmtdesc.index+1,fmtdesc.description);
    fmtdesc.index++;
    }
